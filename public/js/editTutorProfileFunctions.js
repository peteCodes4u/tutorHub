...
changePasswordButton.addEventListener("click", async function () {
  ...
  if (response.ok) {
    ...
    alert("✋ Please ensure your password is at minimum 8 characters long");
  }
});
...
removeInstrumentButton.addEventListener("click", async function () {
  ...
  if (response.ok) {
    ...
    alert(
      "Sorry, it does not appear that this instrument has been associated with your profile in our records 🤔"
    );
  } else if (response.status === 422) {
    alert(
      "Sorry, an error has occurred please ensure you are logged in, refresh the browser and try again 😵"
    );
  }
});
